...
const CountryCode = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res1 = await axios(`${process.env.REACT_APP_BASE_URL}/country`);
                const res2 = await axios.get("https://restcountries.com/v2/all");
                const mergedData = [...res1.data.data, ...res2.data];
                setCountryCodes(mergedData);
    ...
    const page = [
        { page1: 'Ana Sayfa', url1: '/', page2: "Ülke Kodu" },

    ];
    return (
        <div className='w-full m-auto md:w-2/3 my-7 flex flex-col p-3'>
            {page.map((item, index) => (
                        ...
                        {paginatedCountryCodes.map(countryCode => (
                            <tr key={countryCode.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
                                <th scope="col" className="px-6 py-3 w-16">
                                    <img src={countryCode.flag || countryCode.image} alt=' ' />
                                </th>
